...
16. [ ] Final UI consistency pass
   ...
   **Implementation note — 2026-07-29 (Quick Share and Steganography)**
   ...
   Quick Share now opens from `Attach` in a dedicated resizable 760×780 window with a 620×620 minimum instead of expanding inside the narrow Context Assistant. Its advanced settings use a single-column responsive layout with field-level validation, expiration and download presets, true `Until stopped` and `Unlimited` policies, an opt-in custom port, and a readable policy summary. Running shares present their settings read-only until the user explicitly chooses to edit them; port changes remain disabled until the server is stopped, and active links retain the policy captured when they were created. Active sharing is represented in Context Assistant only by a compact status plus `Manage`; link rows preserve filename/status width and expose Copy, Open, and Stop actions.

   The call workspace now uses one canonical `Voice/Video call with <peer>` heading. Start-call buttons disappear while a session is active, leaving the supported `End call` action; the connection disclosure is labelled `Room setup` instead of the ambiguous `Show`. Remote video owns the stage, local video is an overlay, and Fit/Fill, Swap, Details, and Expand controls provide progressive disclosure without repeating call metadata. Peer rows show explicit `Selected` and `In call` badges, Quick Share transcript messages render as actionable Copy/Open events, exact consecutive runtime duplicates are collapsed, and the top bar presents the local identity plus room/connection state. The non-actionable `Security` card was removed from Context Assistant because it duplicated the secure-room status without offering a task or new state. The 200–240 px peer column and 300–320 px Context Assistant bounds preserve the flexible conversation surface; the assistant moves to a drawer below 1280 px.
   ...
   **Implementation note — 2026-07-30 (call layout and transcript density)**

   Peer capability rows now use accessible Voice, Video, and File icons without repeating their text labels. Active video calls support Split view, Focus video, and Focus chat modes; the split divider is draggable and preserves minimum usable video and transcript heights. Overlay controls fade out after pointer inactivity and return on pointer movement, while the local preview can be dragged and snapped to any corner in both embedded and expanded video stages. Presence and generic system events render as compact centered service pills, and join/leave messages are classified before generic system messages. Local LAN addresses no longer enter chat history and instead appear in compact form in the secure-room status. The theme action now uses the same compact button component and height as Settings.
