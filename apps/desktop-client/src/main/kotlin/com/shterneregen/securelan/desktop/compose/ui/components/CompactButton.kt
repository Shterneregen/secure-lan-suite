...
internal fun CompactButton(
    ...
    val backgroundColor = when {
        tone == CompactButtonTone.DESTRUCTIVE && pressed -> tokens.colors.error.copy(alpha = 0.25f)
        tone == CompactButtonTone.DESTRUCTIVE -> tokens.colors.error.copy(alpha = 0.12f)
        pressed -> tokens.colors.accent.copy(alpha = 0.22f)
        tone == CompactButtonTone.SECONDARY -> tokens.colors.surfaceLevel2.copy(alpha = 0.72f)
    ...
    val borderColor = when {
        ...
        tone == CompactButtonTone.DESTRUCTIVE -> tokens.colors.error.copy(alpha = 0.46f)
        pressed -> tokens.colors.accent.copy(alpha = 0.58f)
        tone == CompactButtonTone.SECONDARY -> tokens.colors.accent.copy(alpha = 0.32f)
        else -> tokens.colors.borderSubtle.copy(alpha = 0.48f)
    }
    Button(
        onClick = onClick,
        enabled = enabled,
